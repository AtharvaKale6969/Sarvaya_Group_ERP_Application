...
export default function Designations() {
  ...
  const [selectedDesigForDrawer, setSelectedDesigForDrawer] = useState<any>(null);
  const [viewingEmployee, setViewingEmployee] = useState<any>(null);

  const designations = [
    { id: '1', name: 'Account Executive', count: 1 },
    { id: '2', name: 'Senior Account Executive', count: 0 },
    { id: '3', name: 'Intern', count: 2 },
    { id: '4', name: 'Intern - Data Analyst', count: 1 },
    { id: '5', name: 'Business Analyst', count: 1 },
    { id: '6', name: 'Business Intelligence Analyst', count: 1 },
    { id: '7', name: 'Data Analyst', count: 1 },
    { id: '8', name: 'Executive Data Analyst', count: 1 },
    { id: '9', name: 'Finance Executive', count: 1 },
    { id: '10', name: 'Business Development Associate', count: 8 },
    { id: '11', name: 'Team Lead', count: 1 },
    { id: '12', name: 'Operations Head', count: 1 },
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', backgroundColor: 'white', border: '1px solid #e5e7eb', borderRadius: '8px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', position: 'relative' }}>
      ...
      <div className="header-responsive" style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #e5e7eb' }}>
        <h2 style={{ fontSize: '1.125rem', fontWeight: '600', margin: 0, color: '#374151', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          Designations <span style={{ color: '#0ea5e9' }}>({designations.length})</span>
        </h2>
...
<table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          ...
          <tbody>
            {designations.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((desig) => (
              <tr key={desig.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                ...
                <td style={{ padding: '1rem', fontSize: '0.875rem', color: '#111827' }}>
                  <span 
                    onClick={() => setSelectedDesigForDrawer(desig)}
                ...
                <td style={{ padding: '1rem', textAlign: 'center' }}>
                  <div style={{ position: 'relative', display: 'inline-block' }}>
                    <button 
                      onClick={() => toggleDropdown(`action-${desig.id}`)}
                    ...
                    {activeDropdown === `action-${desig.id}` && (
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', padding: '1.25rem 1.5rem', borderTop: '1px solid #e5e7eb' }}>
        <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>
          Showing <span style={{ fontWeight: '600', color: '#374151' }}>{designations.length === 0 ? 0 : ((currentPage - 1) * pageSize) + 1}</span> to <span style={{ fontWeight: '600', color: '#374151' }}>{Math.min(currentPage * pageSize, designations.length)}</span> of <span style={{ fontWeight: '600', color: '#374151' }}>{designations.length}</span> Results
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', border: '1px solid #d1d5db', borderRadius: '6px', overflow: 'hidden' }}>
            ...
            {Array.from({ length: Math.ceil(designations.length / pageSize) || 1 }).map((_, idx) => (
              <button 
                key={idx}
                ...
                style={{ 
                  ...
                  border: 'none', 
                  borderRight: idx < Math.ceil(designations.length / pageSize) - 1 ? '1px solid #d1d5db' : 'none', 
            ...
            <button 
              onClick={() => setCurrentPage(Math.min(Math.ceil(designations.length / pageSize), currentPage + 1))} 
              style={{ background: 'white', border: 'none', borderLeft: '1px solid #d1d5db', padding: '0.375rem 0.75rem', fontSize: '0.875rem', color: currentPage >= Math.ceil(designations.length / pageSize) ? '#d1d5db' : '#0ea5e9', cursor: currentPage >= Math.ceil(designations.length / pageSize) ? 'default' : 'pointer' }}
            >
              Next
            </button>
          </div>
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', padding: '1.25rem 1.5rem', backgroundColor: '#f0f9ff', borderBottom: '1px solid #e0f2fe' }}>
              <h3 style={{ margin: 0, fontSize: '1.125rem', fontWeight: '600', color: '#0369a1' }}>
                {selectedDesigForDrawer.name} - Employees
              </h3>
              <button 
                onClick={() => setSelectedDesigForDrawer(null)}
            ...
            <div style={{ padding: '1.5rem', overflowY: 'auto', flex: 1 }}>
              <div style={{ marginBottom: '1rem' }}>
                <span style={{ border: '1px solid #d1d5db', borderRadius: '6px', padding: '0.375rem 0.75rem', fontSize: '0.875rem', fontWeight: '500', color: '#374151', display: 'inline-block' }}>
                  Active: {selectedDesigForDrawer.count}
                </span>
              </div>
...
<table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                  ...
                  <tbody>
                    {selectedDesigForDrawer.count === 0 ? (
                      <tr>
                      ...
                      Array.from({ length: selectedDesigForDrawer.count }).map((_, idx) => (
                          ...
                          <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem' }}>
                            <span 
                              onClick={() => setViewingEmployee({
                                ...
                                dept: 'Operations',
                                role: selectedDesigForDrawer.name,
                                status: 'Active',
                                date: '01 Jan 2026'
                              })}
